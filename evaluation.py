...
def evaluate(outputs: Variable, labels:Variable) -> float: 
    #evaluate the newral network outputs against non encoded data 
    Y = labels.numpy() 
    Y_hat = np.argmax(outputs, axis=1)
    return float(np.sum(Y_hat == Y))

def batch_evaluation(net: Net, dataloader: torch.utils.data.DataLoader) -> float: 
...
def validate(): 
    ...
    pretrained_model = torch.load("model_params.pth")
